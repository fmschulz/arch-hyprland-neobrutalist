...
CONFIG_DIR=/etc/arch-hypr-neobrutalist
CONFIG_FILE=${CONFIG_DIR}/usb-automount.conf
RULES_FILE=/etc/udev/rules.d/99-arch-hypr-neobrutalist-usb-automount.rules
...
STATE_DIR="/run/arch-hypr-neobrutalist-usb"
EOF_CONF
...
ACTION=="add", SUBSYSTEM=="block", ENV{DEVTYPE}=="partition", ENV{ID_BUS}=="usb", ENV{ID_FS_USAGE}=="filesystem", RUN+="/usr/local/lib/arch-hypr-neobrutalist/usb-automount.sh add %k"
ACTION=="remove", SUBSYSTEM=="block", ENV{DEVTYPE}=="partition", ENV{ID_BUS}=="usb", RUN+="/usr/local/lib/arch-hypr-neobrutalist/usb-automount.sh remove %k"
EOF_RULES
